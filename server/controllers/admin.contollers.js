...
const doctorModel = require('../models/user.models')
...
const getAllDoctors = async (req, res) => {
  try {
    const doctors = await doctorModel.find({ hospitalId: req.user.hospitalId, role: 'doctor' }).select('-password -passwordHash').lean().exec();
...
const getAllPatients = async (req, res) => {
  try {
    const patients = await patientModel.find({ hospitalId: req.user.hospitalId, role: 'patient' }).lean().exec();
