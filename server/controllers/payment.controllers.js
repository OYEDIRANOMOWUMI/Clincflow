...
const createPaymentCheckout = async (req, res) => {
  try {
    ...
    const patient = await User.findOne({ _id: targetPatientId, hospitalId: req.user.hospitalId, role: 'patient' })
    if (!patient) {
      return res.status(404).json({ success: false, message: 'Patient not found' })
    }

    const prescription = await Prescription.findOne({ _id: prescriptionId, hospitalId: req.user.hospitalId })
    if (!prescription) {
      return res.status(404).json({ success: false, message: 'Prescription not found' })
    }

    if (String(prescription.patient) !== String(targetPatientId)) {
...
const getPaymentsForPatient = async (req, res) => {
  try {
    ...
    const prescriptionIds = await Prescription.find({ hospitalId: req.user.hospitalId, patient: targetPatientId }).distinct('_id')
    const payments = await Payment.find({ patient: targetPatientId, prescription: { $in: prescriptionIds } })
