...
const createPrescription = async (req, res) => {
  try {
    ...
    const patientDoc = await User.findOne({ _id: patient, hospitalId: req.user.hospitalId, role: 'patient' })
    if (!patientDoc) {
      return res.status(404).json({ success: false, message: 'Patient not found' })
    }

    const doctorDoc = await User.findOne({ _id: doctorId, hospitalId: req.user.hospitalId, role: 'doctor', isActive: true })
    if (!doctorDoc) {
      return res.status(404).json({ success: false, message: 'Doctor not found' })
    }

    if (appointment) {
      const appointmentDoc = await Appointment.findOne({ _id: appointment, hospitalId: req.user.hospitalId })
      if (!appointmentDoc) {
...
const getPrescriptions = async (req, res) => {
  try {
    ...
    let filter = { hospitalId: req.user.hospitalId }
    const role = normalizeRole(req.user.role)
    ...
    if (role === 'patient') {
      ...
      filter = { hospitalId: req.user.hospitalId, status: { $in: ['unpaid', 'paid', 'dispensed'] } }
    }
