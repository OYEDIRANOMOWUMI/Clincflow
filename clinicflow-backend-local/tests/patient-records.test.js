...
const result = buildPatientUpdate({ diagnosis: 'Improved blood pressure', notes: 'Follow-up in one week' }, {
  diagnosis: 'Initial evaluation',
  notes: 'Needs review'
});

assert.equal(result.diagnosis, 'Improved blood pressure');
assert.equal(result.notes, 'Initial evaluation | Follow-up in one week');
