...
const sendAppointmentNotifications = async (appointment) => {
  ...
  const messageBody = `Hello ${appointment.patientName || 'patient'}, your appointment request for ${appointment.department || 'ClinicFlow'} has been received. Our team will contact you shortly.`
...
const createAppointment = async (req, res) => {
  try {
    ...
    if (appointmentPayload.patient && appointmentPayload.patient !== null) {
      const patientUser = await User.findById(appointmentPayload.patient).lean()
      if (!patientUser) {
    ...
    if (appointmentPayload.doctor && appointmentPayload.doctor !== null) {
      const doctorUser = await User.findById(appointmentPayload.doctor).lean()
      if (!doctorUser) {
        return res.status(404).json({
          success: false,
